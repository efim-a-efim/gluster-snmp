...
class PassPersist:
    ...
    def __init__(self, base_oid, log=None):
        """
    ...
    def main_passpersist(self):
        ...
        if "PING" in line:
            ...
            if oid is None:
                ...
                self.log(self.get_first())
                out = self.get_first()
            else:
                self.log(self.get_next(oid))
                out = self.get_next(oid)
        elif 'GET' in line:
            oid = self.cut_oid(sys.stdin.readline().strip())
            if oid is None:
                out = "NONE"
            else:
                self.log(self.get(oid))
                out = self.get(oid)
        elif 'SET' in line:
            oid = sys.stdin.readline().strip()
            typevalue = sys.stdin.readline().strip()
            out = self.set(oid, typevalue)
        elif 'DUMP' in line:
            try:
